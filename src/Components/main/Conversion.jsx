...
const Conversion = () => {
    ...
    const currency = ['INR', 'USD', 'JPY', 'SGD', 'EUR', 'GBP', 'AUD', 'CAD', 'CHF', 'HKD', 'NZD'];
    const [selectedCurrency1, setSelectedCurrency1] = useState(localStorage.getItem('selectedCurrency1') || 'INR');
    const [selectedCurrency2, setSelectedCurrency2] = useState(localStorage.getItem('selectedCurrency2') || 'USD');
    ...
    const fetchExchangeRate = async () => {
        if (selectedCurrency1 && selectedCurrency2) {
            ...
            try {
                ...
                const rate = data.data[selectedCurrency2] / data.data[selectedCurrency1];
 ...
 console.log(selectedCurrency1);
  ...
  return (
    <>
    <Card className='card'>
        <Typography sx={{p:"0.8em 0 0.5em 1.2em", color:"#989898", fontSize:"0.9em"}}>Amount</Typography>
        <Box className="boxCont">
            <Box className="boxAmo" gap={2}> 
                <Avatar alt={selectedCurrency1} src={`${selectedCurrency1}`} className='avatar' />
                <Typography>{selectedCurrency1}</Typography>
                <KeyboardArrowDownIcon onClick={handleToggle1}/>
            </Box>
                <TextField type='number' className='text' 
                   value={inputValue1}
        ...
        <Box className="drop">
          {currency.map((currency, index) => (
            <Box key={index} className="dropItem" onClick={() => handleCurrency1(currency)}>
        ...
        <Typography sx={{p:"0.8em 0 0.5em 1.2em", color:"#989898", fontSize:"0.9em"}}>Converted Amount</Typography>
        <Box className="boxCont">
            <Box className="boxAmo" gap={2}> 
                <Avatar alt={selectedCurrency2} src={require(`../images/${selectedCurrency2}.png`).default} className='avatar' />
                <Typography>{selectedCurrency2}</Typography>
        ...
        <Box className="drop">
          {currency.map((currency, index) => (
            <Box key={index} className="dropItem" onClick={() => handleCurrency2(currency)}>
              <Typography>{currency}</Typography>
            </Box>
          ))}
        </Box>
    ...
    {!showKeypad && (
         <Box>
            <Typography sx={{p:"1em 0 0.5em 1.8em", color:"#989898", fontSize:"0.9em"}}>Indicative Exchange Rate</Typography>
            <Typography sx={{pl:"1.5em", fontSize:"1.2em", fontWeight:"600"}}> 1 {selectedCurrency1} = {exchangeRate.toFixed(4)} {selectedCurrency2}</Typography>
        </Box>
    )}
    ...
    <div className="keypad">
      {buttons.map((button, index) => (
        <Card key={index} className="key" onClick={() => handleKeypadClick(button.number)}>
